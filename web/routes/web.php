...
use Gnikyt\BasicShopifyAPI\Session as APISession;
...
Route::post('/api/shipping/getrate', function (Request $request) {
    ...
    if (empty($credential))
        return false;
    $token = Http::post('https://lb.icarry.com/api-frontend/Authenticate/GetTokenForCustomerApi', [
    ...
    ])->post('https://lb.icarry.com/api-frontend/SmartwareShipment/EstimateRatesByCOD', [
...
Route::post('/api/shipping/create_order', function (Request $request) {
    ...
    $create_orders = Http::withHeaders([
        'Authorization' => 'Bearer ' . $token->token,
    ])->post('https://lb.icarry.com/api-frontend/SmartwareShipment/CreateOrder', [
        "ProcessOrder" => false,
        "pickupLocation" => $location_address,
        "dropOffAddress" => [
            "FirstName" => $order['shipping_address']['first_name'],
    ...
    $tracking_info = array(
        ...
        "url" => "https://lb.icarry.com/Order/TraceShipment?trackingNumber=" . $tracking_number,
        "company" => "iCARRY"
    );
...
Route::get('/api/configuration', function (Request $request) {
    ...
    try {
        ...
        $carrier_service->name = $CARRIER_SERVICE_NAME;
...
Route::post('/api/configuration/check', function (Request $request) {
    ...
    $data = Http::post('https://lb.icarry.com/api-frontend/Authenticate/GetTokenForCustomerApi', [
        'Email' => $email,
        'Password' => $password
    ])->object();
